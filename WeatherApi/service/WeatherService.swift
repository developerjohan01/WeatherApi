...
class WeatherService {
    ...
    fileprivate func buildUrlString(_ city: String?, _ longitude: Double?, _ latitude: Double?) -> String? {
        ...
        if longitude != nil && latitude != nil {
            ...
            // default e.g. Cape Town url += "lon=18.42&lat=-33.93&units=metric&appid="
            return nil
        }
        url += apiKey
        return url
    ...
    func fetchForcast(city: String?, longitude: Double?, latitude:Double?) -> [Forecast] {
        ...
        locationCityName = Constants.unknown
        locationLatitude = 0.0
        locationLongitude = 0.0
        
        if let urlString = buildUrlString(city, longitude, latitude) {
            print(urlString)
            ...
            let task = URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                do {
                    ...
                    if let name = nameResult.string {
                        ...
                        self.locationCityName = Constants.unknown
                    ...
                    // Force unwrap of city name
                    self.location = Location(name: self.locationCityName!, latitude: self.locationLatitude , longitude: self.locationLongitude )
